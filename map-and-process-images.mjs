...
const catMap = {
  ...
  "sparklers": "sparklers",
  "special colour sparklers": "special colour sparklers",
  "ground chakkar": "ground chakkar",
  "spcial color chakkar": "spcial color chakkar",
  "flower pots": "flower pots",
  "one sound crackers": "one sound crackers",
  "bombs": "bombs",
  "paper bomb": "paper bomb",
  "twinkle star": "twinkle star",
  "special novelties": "special novelties",
  "kids novelties": "kids novelties",
  "colour fountain": "colour fountain",
  "peacock fountain": "peacock fountain",
  "colour sticks": "colour sticks",
  "rockets": "rockets",
  "new arrivals 2026": "new arrivals 2026",
  "loose crackers": "loose crackers",
  "single outs": "single outs",
  "single ariel outs": "single ariel outs",
  "special colour single aireal outs": "special colour single aireal outs",
  "combo ariel out": "combo ariel out",
  "repeating multi colour shots": "repeating multi colour shots",
  "special multi colour shot 2026": "special multi colour shot 2026",
  "festival display": "festival display",
  "digital lar": "digital lar",
  "colour matches": "colour matches",
  "guns": "guns",
  "gift box": "gift box",
  "vanitha fireworks special fancy outs": "vanitha fireworks special fancy outs",
  "sonny fancy": "sonny fancy"
};
...
for (let i = 0; i < lines.length; i++) {
    ...
    if (catMap[lower] && !lower.startsWith('₹') && !lower.startsWith('total:')) {
        currentCategory = lower;
        dbProductsMap[currentCategory] = [];
        nameBuffer = [];
        continue;
    }
    ...
    if (lower.startsWith('total:')) {
        ...
        if (name) {
            dbProductsMap[currentCategory].push(name);
        }
        nameBuffer = [];
    } else {
...
const folderToCategory = {
    "Bombs 14": "bombs",
    "Color Matches 29": "colour matches",
    "Combo Ariel Fancy 20": "combo ariel out",
    "DIGITEL LAR": "digital lar",
    "Fancy Fountain Multi Color 10": "colour fountain",
    "Fancy Novelties 09": "special novelties",
    "Fancy Wheels 04": "spcial color chakkar",
    "Festival Display Set Out 26": "festival display",
    "Flower Pots 02": "flower pots",
    "Gift Box 30": "gift box",
    "Ground Chakkar 03": "ground chakkar",
    "guns 29": "guns",
    "Kids Novelties 08": "kids novelties",
    "Loose Crackers 16": "loose crackers",
    "New Arrivals 2026 13": "new arrivals 2026",
    "One Sound Crackers 01": "one sound crackers",
    "Paper Bombs 15": "paper bomb",
    "Peacock Fountain 12": "peacock fountain",
    "Repeating Multi Color Shots 23": "repeating multi colour shots",
    "Rockets 07": "rockets",
    "Single Ariel Fancy (5 Pcs) 17": "single outs",
    "Single Ariel Fancy 18": "single ariel outs",
    "Sonny Fancy 22": "sonny fancy",
    "Sparklers 27": "sparklers",
    "Special Color Ariel Fancy 19": "special colour single aireal outs",
    "Special Color Sparklers 28": "special colour sparklers",
    "Special Fountain Exotic Series 11": "special multi colour shot 2026",
    "Special Multi Color Shots 2026 24": "special multi colour shot 2026",
    "Torches & Pencils 06": "colour sticks",
    "Twinkling Star 05": "twinkle star",
    "vanatha 31": "vanitha fireworks special fancy outs"
};
...
async function processImages() {
    ...
    for (const dir of dirs) {
        let catKey = folderToCategory[dir];
        if (!catKey) continue;
        
        let products = dbProductsMap[catKey] || [];
        const files = fs.readdirSync(path.join(sourceDir, dir)).filter(f => /\.(jpg|jpeg|png|webp|avif)$/i.test(f)).sort();
            
        console.log(`Folder ${dir} -> Cat ${catKey} | Images: ${files.length} | Products: ${products.length}`);
        
        const max = Math.min(files.length, products.length);
        for (let i = 0; i < max; i++) {
            const file = files[i];
            ...
            const fullPath = path.join(sourceDir, dir, file);
            ...
            try {
                ...
                updates.push({
                    name: productName,
                    imageUrl: `/images/products/${destName}`
                });
            } catch (e) {
    ...
    fs.writeFileSync('db-image-updates.json', JSON.stringify(updates, null, 2));
