...
# Replace the CTA section using regex or precise substring
old_cta_pattern = re.compile(r"\{\/\*\ 2\.\ PREMIUM\ SHOP\ NOW\ CTA\ \*\/.*?<\/section>", re.DOTALL)

new_cta = """{/* 2. PREMIUM SHOP NOW CTA */}
        <section className="w-full pt-8 pb-3 sm:pt-12 sm:pb-5 flex justify-center items-center bg-white relative z-10">
          <div className="animate-[floatMobile_4s_ease-in-out_infinite]">
            <Link 
              href="/shop"
              className="group relative inline-flex items-center justify-center gap-2.5 px-8 py-3.5 sm:px-10 sm:py-4 bg-gradient-to-r from-[#dca42b] via-[#fce074] to-[#dca42b] rounded-full font-bold text-xs sm:text-sm text-gray-900 shadow-[0_0_15px_rgba(220,164,43,0.4)] hover:shadow-[0_8px_25px_rgba(220,164,43,0.7)] transition-all duration-500 hover:-translate-y-1 hover:scale-105 active:scale-95 border border-[#fef1a7] overflow-hidden"
            >
...
content = re.sub(old_cta_pattern, new_cta, content)
